...
class WhatsAppUserSettingsController extends BaseController
{
    ...
    /**
     * Update WhatsApp token (user-level)
     */
    ...
    /**
     * Update WhatsApp business profile (user-level)
     */
    public function whatsappBusinessProfileUpdate(StoreWhatsappProfile $request)
    {
        ...
        if (isset($config['whatsapp'])) {
            // Check user permissions for business profile updates
            if (!$this->canUpdateBusinessProfile($workspace)) {
                return back()->with(
                    'status', [
                        'type' => 'error',
                        'message' => __('You do not have permission to update business profile.')
                    ]
                );
            }
    ...
    /**
     * Delete WhatsApp integration (user-level)
     */
    public function deleteWhatsappIntegration(Request $request)
    {
        ...
        try {
            ...
            if (isset($metadata['whatsapp'])) {
                // Check user permissions for deleting integration
                if (!$this->canDeleteWhatsAppIntegration($workspace)) {
                    return back()->with(
                        'status', [
                            'type' => 'error',
                            'message' => __('You do not have permission to delete WhatsApp integration.')
                        ]
                    );
                }
    ...
    /**
     * Get WhatsApp user settings status
     */
    public function getWhatsAppStatus(Request $request)
    {
        ...
        try {
            ...
            $status = [
                ...
                'can_update_profile' => $this->canUpdateBusinessProfile($workspace),
                'can_delete_integration' => $this->canDeleteWhatsAppIntegration($workspace),
            ];
    ...
    public function getWhatsAppTemplates(Request $request)
    {
        ...
        try {
            $templates = Template::where('workspace_id', $workspaceId)
                ...
                ->get(['id', 'name', 'category', 'language', 'status']);
    ...
    /**
     * Save WhatsApp settings - helper method
     */
    ...
    /**
     * Check if user can delete WhatsApp integration
     */
     ...
     */
    private function demoModeResponse()
    {
        return back()->with(
            'status', [
                'type' => 'error',
