    ...
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-gray-900">{{ $t('WhatsApp Numbers') }}</h1>
                    <p class="mt-2 text-sm text-gray-700">
                        {{ $t('Connect multiple WhatsApp numbers via QR code for multi-number messaging') }}
                    </p>
                </div>
...
onMounted(() => {
    ...
    if (window.Echo) {
        echo = window.Echo
        const channel = echo.channel(`workspace.${window.workspaceId}`)

        channel.listen('qr-code-generated', handleQRGenerated)
        channel.listen('session-status-changed', handleSessionStatusChanged)
    }
})
...
onUnmounted(() => {
    ...
    if (echo && window.workspaceId) {
        echo.leaveChannel(`workspace.${window.workspaceId}`)
    }
})

const handleQRGenerated = (data) => {
    if (data.workspace_id === window.workspaceId) {
...
const setPrimary = async (uuid) => {
    try {
        ...
        alert('Failed to set primary session')
    }
}
