<template>
    <div :class="rtlClass">
        <div class="flex h-screen w-full tracking-[0.3px] bg-gray-300/10">
            ...
            <div class="min-h-screen flex flex-col w-full min-w-0 items-center justify-center space-x-4">
                <div class="bg-white rounded-xl p-5 border">
                    <h4 class="font-regular text-xl mb-4 text-center">{{ $t('Select workspace') }}</h4>

                    <div class="min-w-[300px] border-t pt-4 space-y-4">
                        <div v-for="(item, index) in props.workspaces" :key="index" @click="selectOrganization(item.workspace.uuid)" class="flex space-x-8 bg-white rounded-md px-4 py-2 justify-between items-center w-full hover:shadow-md cursor-pointer">
                        ...
                        <div @click="isOpenModal = true" class="flex space-x-8 bg-white border rounded-md px-4 py-2 justify-between items-center w-full hover:bg-slate-100 cursor-pointer">
                            <div class="w-full">
                                <h2 class="font-light text-center text-sm">Create workspace</h2>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <WorkspaceModal v-model:modelValue="isOpenModal"/>
    </div>
</template>
<script setup>
    import { ref } from 'vue';
