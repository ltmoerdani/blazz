    ...
    <Modal :label="$t('Switch teams')" :isOpen="isLocationSwitchModalOpen">
        <div class="mt-2 grid grid-cols-1 gap-x-6">
            <div class="pt-3 space-y-2 text-sm">
                <div v-for="(item, index) in props.workspaces" :key="index" @click="selectOrganization(item.workspace.uuid)" class="flex gap-x-8 hover:bg-slate-200 rounded-lg py-1 justify-between items-center w-full cursor-pointer border border-slate-100 pl-1 pr-2">
...
<script setup>
    ...
    const props = defineProps(['config', 'user', 'workspace', 'organizations', 'isSidebarOpen', 'menuIconsOnly', 'unreadMessages']);
