<template>
    <aside class="md:flex flex-col h-full bg-white hidden relative" :class="menuIconsOnly ? 'w-20' : 'w-80'">
        ...
        <Menu :config="props.config" :user="props.user" :unreadMessages="unreadMessages" :workspace="props.workspace" :organizations="props.workspaces" :menuIconsOnly="menuIconsOnly"></Menu>
    </aside>
</template>
<script setup>
    ...
    const props = defineProps(['user', 'workspace', 'organizations', 'config', 'unreadMessages']);
